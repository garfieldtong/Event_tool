...
async def return_whole_csv():
    ...
    try:
        ori_df = pd.read_csv("app/circle_list.csv")
...
async def reset_csv():
    ...
    df = pd.read_csv("app/original.txt")
    df.to_csv("app/circle_list.csv", index = False)
    ori_df = pd.read_csv("app/circle_list.csv")
    df = ori_df.to_html(index = False).replace("\n", "").replace("NaN", "")
...
@app.post("/add_circle/")
async def add_circle(request: Request):
    input = await request.body()
    ...
    try:
        ...
        pd.concat([ori_df, appendRow], ignore_index=True).to_csv("app/circle_list.csv", index = False)
...
async def change_bought_status(request: Request):
    ...
    try:
        ...
        ori_df.to_csv("app/circle_list.csv", index = False)
